...
export const create = async (req: Request, res: Response) => {
  ...
  const existing = safeId
    ? await db
        .select()
        .from(plannerTasks)
        .where(and(eq(plannerTasks.id, safeId), eq(plannerTasks.userId, uid)))
    : [];
...
export const deleteById = async (req: Request, res: Response) => {
  ...
  await db
    .delete(plannerTasks)
    .where(and(eq(plannerTasks.id, req.params.id), eq(plannerTasks.userId, uid)));
  res.json({ ok: true });
};
...
export const postSeed = async (req: Request, res: Response) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const learnSeeds =
      week === 1 ? W1_LEARN : week === 2 ? W2_LEARN : week === 3 ? W3_LEARN : W4_LEARN;
